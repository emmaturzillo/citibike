...
- **Data Cleaning:** Cleaned station ID strings and imputed missing station IDs using station name where possible. Dropped records where station ID could not be determined, as well as records with missing start and/or end times.
- **Data Analysis:** Evaluated distribution of Citi Bike frequency, time cost (trip duration), and financial cost at the block group level. Assessed relationship between these variables and block groups's 1) median household income, and 2) % of population that is white. Used ranked regression in all cases.
- **Reporting:** Rendered full analytical report via Quarto to combine code execution, interactivity, and narrative text.
